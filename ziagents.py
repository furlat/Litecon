...
class Trade(BaseModel):
    ...
    buyer_value: float
    seller_cost: float
    round: int  # Add this attribute to store the round number
...
class ZIAgent(BaseModel):
    ...
    def calculate_trade_surplus(self, trade: Trade) -> float:
        if trade.buyer_id == self.id:
            # Buyer: Calculate surplus as the difference between the buyer's value and the trade price
            ...
            # Seller: Calculate surplus as the difference between the trade price and the seller's cost
    ...
    def generate_bid(self) -> Optional[Order]:
        ...
        if is_buy:
            ...
            # Ensure the bid does not exceed the buyer's value and the buyer can afford it
            if base_value <= 0 or available_cash < base_value:
                return None

            price = random.uniform(base_value * (1 - self.max_relative_spread), base_value)
            price = min(price, available_cash, base_value)  # Make sure the bid does not exceed base_value
            ...
            return Order(
                ...
                quantity=1,
                price=price,
                base_value=base_value  # Pass the buyer's value for this quantity
            )
            ...
            # Ensure the ask is above the seller's cost
            if base_cost <= 0:
                return None

            price = random.uniform(base_cost, base_cost * (1.0 + self.max_relative_spread))
            price = max(price, base_cost)  # Ensure the ask is not below base_cost
            ...
            return Order(
                ...
                quantity=1,
                price=price,
                base_cost=base_cost  # Pass the seller's cost for this quantity
            )

    def finalize_trade(self, trade: Trade):
        """Finalize the trade by updating the agent's allocation of cash and goods."""
        if trade.buyer_id == self.id:
            cash_change = trade.price * trade.quantity
            self.allocation.cash = round(self.allocation.cash - cash_change, 10)  # Round to 10 decimal places
    ...
    def respond_to_order(self, order: Order, accepted: bool):
        """Respond to the environment's acceptance or rejection of the order."""
        if accepted:
    ...
    def plot_order_history(self):
        """Plot the history of posted orders."""
    ...
    def calculate_individual_surplus(self) -> float:
        """Calculate the individual's surplus based on their final allocation."""
        if self.preference_schedule.is_buyer:
            # Buyers: Surplus is the difference between the value of goods obtained and cash spent
            ...
            # Sellers: Surplus is the difference between cash earned and the cost of goods sold
...
def run_simulation(agent: ZIAgent, num_rounds: int):
    ...
    for _ in range(num_rounds):
        ...
        if order:
            # Determine the value or cost based on whether the agent is a buyer or seller
            buyer_value = order.base_value if order.is_buy else None
            seller_cost = order.base_cost if not order.is_buy else None
            
            # Create a Trade object with the necessary fields
            trade = Trade(
                buyer_id=order.agent_id if order.is_buy else -1,
                seller_id=-1 if order.is_buy else order.agent_id,
                quantity=order.quantity,
                price=order.price,
                buyer_value=buyer_value if buyer_value else 0.0,
                seller_cost=seller_cost if seller_cost else 0.0
            )
            
            # Simulate environment response (for simplicity, always accept the order)
            agent.finalize_trade(trade)
            agent.respond_to_order(order, accepted=True)
        else:
            # Log rejected orders as well
